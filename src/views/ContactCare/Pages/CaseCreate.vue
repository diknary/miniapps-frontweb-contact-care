      ...
      <form v-on:submit.prevent>
        ...
        <div class="d-flex justify-content-center mt-2">
          ...
          <base-button
            type="button"
            isSecondary
            :isDisabled="dataKontrak.noTiket && isSent ? false : true"
...
export default {
  ...
  data() {
    return {
      ...
      isNoKontrakDisabled: false,
      isNoPolDisabled: false,
      isMerkDisabled: false,
      isWarnaDisabled: false,
      isTipeDisabled: false,
      isJenisPembiayaanDisabled: false,
      isTipePembiayaanDisabled: false,
      isBankPendanaanDisabled: false,
      isCabangBookingDisabled: false,
      isCabangHandlingDisabled: false,
      isTanggalLahirDisabled: false,
      isAlamatKTPDisabled: false,
      isIbuKandungDisabled: false,
      subKategoriFilter: [],
      organisasiFilter: [],
      dataKontrak: {
        id: "",
  ...
  async created() {
    ...
    const promises = await Promise.all([responseSumberTiket, responseKategori]);
  ...
  methods: {
    ...
    onChangeBagianDept(){
      ...
      const filterPic = [
        ...new Map(
          getDept.map((item) => [item["nikPic"], item])
        ).values(),
      ];
    ...
    onChangeSubKategori() {
      ...
      this.dataKontrak.slaTipekasus = subkategori[0].domainDesc4;
    },
    ...
    async send() {
      ...
      this.dataKontrak.flag = "1";
      this.dataKontrak.sendBy = this.getNik;
      this.dataKontrak.cabangLogin = this.getBranchId;
      const response = await ContactCareSvc.saveTiket(this.dataKontrak);
      ...
      if (response.status == 200) {
        ...
        this.dataKontrak.noTiket = response.data.data.nomorTiket;
        this.dataKontrak.id = response.data.data.id;
        this.dataKontrak.dibuatPada = response.data.data.dibuatPada;
