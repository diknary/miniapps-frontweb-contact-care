...
//import EGRCommAci from "@/service/EvaluatorGR/common/commAci";
    ...
    async initializeApp() {
      ...
      try {
        ...
        if (
          ...
          // this.setJobMatrix(nik, brid, [
          //     { applicationAlias: "IDM.EVALUATORGR.DATAPARKIR" },
          //     { applicationAlias: "IDM.EVALUATORGR.REPORT" },
          //   ]);
          const response = await CommAciSvc.getMenuReportAci();
          if (response.status == 200) {
            ...
            //   response.data.data
            // );
            const listMenuParent = listMenuAkses.filter(
              (item) => item.value5 === "P"
            );

            const listMenuNonParent = listMenuAkses.filter(
              (item) => !item.value5 || (item.value5 != "P" && item.value5 != "C")
            );

            for (const data of listMenuParent) {
              const listMenuChild = listMenuAkses.filter(
                (item) =>
                  item.value5 === "C" && item.value6 === data.value6
              );

              const menuChild = listMenuChild.map((c) => ({
                href: c.value1,
                title: c.value2,
                source: c.value3,
              }));
            ...
            for (const data of listMenuNonParent) {
            ...
            this.$router.push("/report/pdk/cabang").catch(() => {});
          } else {
