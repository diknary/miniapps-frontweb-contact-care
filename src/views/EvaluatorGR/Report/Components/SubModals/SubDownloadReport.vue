      ...
      <div class="data-kontrak-wrapper">
        <div class="data-kontrak__detail">
          <div class="form-group row">
            ...
            <div class="col-sm-5">
              <date-picker id="dateFrom" v-model="periodeFrom" />
            </div>
            <div class="col-sm-5">
              <date-picker id="dateTo" v-model="periodeTo" />
            </div>
          </div>
          <div class="form-group row">
            <div class="col-sm-2">
...
export default {
  ...
  methods: {
    generateFile(binaryBlob) {
      const filename = "Report_Evaluator_" +
        ".xlsx";
    ...
    async download() {
      ...
      if(this.periodeFrom.value && this.periodeTo.value){
          formattedFrom = format(new Date(this.periodeFrom.value), 'dd-MM-yyyy');
          formattedTo = format(new Date(this.periodeTo.value), 'dd-MM-yyyy');
      }
      console.log(formattedFrom)
      const response = await ReportSvc.downloadReportGr('', formattedFrom, formattedTo, this.status.value);
      if (response){
  ...
  computed: {
    isAllValid() {
      return (
        this.periodeFrom.isValid &&
        this.periodeTo.isValid &&
        this.status.isValid
      );
    },
  },
